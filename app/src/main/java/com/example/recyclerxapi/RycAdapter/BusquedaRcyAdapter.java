...
import com.example.recyclerxapi.Activities.InicioActivity;
import com.example.recyclerxapi.ApiThings.Animes;
import com.example.recyclerxapi.R;

import java.util.ArrayList;

public class BusquedaRcyAdapter extends RecyclerView.Adapter<BusquedaRcyAdapter.ViewHoldar> {
    Context contexto;
    ArrayList<Animes.Anime> animes;

    public BusquedaRcyAdapter(Context context, ArrayList<Animes.Anime> lista) {
    ...
    @NonNull
    @Override
    public BusquedaRcyAdapter.ViewHoldar onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        LayoutInflater inflador = LayoutInflater.from(contexto);
        View view = inflador.inflate(R.layout.vw_elemento, parent, false);
        return new BusquedaRcyAdapter.ViewHoldar(view);
    }
    ...
    public void onBindViewHolder(@NonNull ViewHoldar holder, int position) {
        ...
        holder.animeCard.setOnClickListener(v -> {
            Intent intent = new Intent(contexto, InicioActivity.class);
            intent.putExtra("animeId", anime.getAnimeID());
            intent.putExtra("animeName", anime.getAtributos().getAnimeName());
            intent.putExtra("animeState", anime.getAtributos().getAnimeState());
            intent.putExtra("animeCaps", anime.getAtributos().getAnimeCaps());
