...
public class FinalizadoRycAdapter extends RecyclerView.Adapter<FinalizadoRycAdapter.ViewHoldar> {

    Context contexto;
    ArrayList<Animes.Anime> animes;

    public FinalizadoRycAdapter(Context contexto,ArrayList<Animes.Anime>listAnimes){
    ...
    @NonNull
    @Override
    public FinalizadoRycAdapter.ViewHoldar onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        LayoutInflater inflador = LayoutInflater.from(contexto);
        View view = inflador.inflate(R.layout.vw_elemento, parent, false);
        return new FinalizadoRycAdapter.ViewHoldar(view);
    }

    @Override
    public void onBindViewHolder(@NonNull FinalizadoRycAdapter.ViewHoldar holder, int position) {

        Animes.Anime anime=animes.get(position);

        holder.animeCard.setOnClickListener(v -> {
            Intent intent = new Intent(contexto, FinalizadosActivity.class);
