...
import com.example.recyclerxapi.Activities.ListaSavedActivity;
import com.example.recyclerxapi.ApiThings.Animes;
import com.example.recyclerxapi.R;

import java.util.ArrayList;

public class ListSavedAdapter extends RecyclerView.Adapter<ListSavedAdapter.ViewHoldar> {

    Context contexto;
    ArrayList<Animes.Anime> animes;

    public ListSavedAdapter(Context contexto,ArrayList<Animes.Anime>listAnimes){
    ...
    @NonNull
    @Override
    public ListSavedAdapter.ViewHoldar onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        LayoutInflater inflador = LayoutInflater.from(contexto);
        View view = inflador.inflate(R.layout.vw_elemento, parent, false);
        return new ListSavedAdapter.ViewHoldar(view);
    }

    @Override
    public void onBindViewHolder(@NonNull ListSavedAdapter.ViewHoldar holder, int position) {

        Animes.Anime anime=animes.get(position);

        holder.animeCard.setOnClickListener(v -> {
            Intent intent = new Intent(contexto, ListaSavedActivity.class);
